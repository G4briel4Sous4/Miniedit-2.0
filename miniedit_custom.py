...
###########################################################################################################################################################################################
    ...
    def createMenubar( self ):
        ...
        fileMenu.add_command( label="Load Template", command=self.load_template)
        fileMenu.add_separator()
        ...
        # Adiciona o menu Tools
        toolsMenu = Menu(mbar, tearoff=False)
        mbar.add_cascade(label="Tools", font=font, menu=toolsMenu)

        # Adiciona o comando Ping
        toolsMenu.add_command(label="Ping", font=font, command=self.run_ping)

        # Adiciona o comando TCPdump
        toolsMenu.add_command(label="TCPdump", font=font, command=self.run_tcpdump)

        # Adiciona o comando Iperf
        toolsMenu.add_command(label="Iperf", font=font, command=self.run_iperf)
    # Canvas
    ...
    def run_ping(self):
        """Executa o comando Ping entre dois hosts selecionados."""
        if not self.net:
            messagebox.showerror("Erro", "Nenhuma topologia carregada.")
            return

        # Abre uma janela para selecionar os hosts
        hosts = self.select_hosts("Selecione dois hosts para o Ping", max_selection=2)
        if not hosts or len(hosts) != 2:
            return

        host1, host2 = hosts
        print(f"Executando Ping de {host1} para {host2}...")
        result = self.net[host1].cmd(f"ping -c 4 {self.net[host2].IP()}")
        print(result)
        messagebox.showinfo("Ping Resultado", result)

    def run_tcpdump(self):
        """Executa o comando TCPdump em um host selecionado."""
        if not self.net:
            messagebox.showerror("Erro", "Nenhuma topologia carregada.")
            return

        # Abre uma janela para selecionar o host
        hosts = self.select_hosts("Selecione um host para o TCPdump", max_selection=1)
        if not hosts:
            return

        host = hosts[0]
        print(f"Executando TCPdump no host {host}...")
        pid = self.net[host].cmd("tcpdump -i any -w capture.pcap & echo $!")
        print(f"TCPdump iniciado no host {host} com PID {pid}")
        messagebox.showinfo("TCPdump", f"TCPdump iniciado no host {host} com PID {pid}")

    def run_iperf(self):
        """Executa o comando Iperf entre dois hosts selecionados."""
        if not self.net:
            messagebox.showerror("Erro", "Nenhuma topologia carregada.")
            return

        # Abre uma janela para selecionar os hosts
        hosts = self.select_hosts("Selecione dois hosts para o Iperf", max_selection=2)
        if not hosts or len(hosts) != 2:
            return

        host1, host2 = hosts
        print(f"Executando Iperf entre {host1} e {host2}...")
        # Inicia o servidor Iperf no host2
        self.net[host2].cmd("iperf -s &")
        # Executa o cliente Iperf no host1
        result = self.net[host1].cmd(f"iperf -c {self.net[host2].IP()}")
        print(result)
...
###########################################################################################################################################################################################
            ...
            if self.cycleDetect: print("Há ciclos na topologia!")
            self.g = self.G.copy()
